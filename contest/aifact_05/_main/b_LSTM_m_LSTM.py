...
for i,v in enumerate(test_csv_list[0:3]) : 
    a= 0
    print(f'시작 : test_csv_list{i}')
    for i02 in range(65) :
        print(f'하루 : {i02}')
        for v02 in aws_col_list : 
            for i03_01 in range(72) : 
                timesteps = 24
                # 1. 데이터
                # 전처리
                print('전처리')
                ...
                le=LabelEncoder()
                v['지점'] = le.fit_transform(v['지점'])

                v['month'] = v['일시'].str[:2]
                v['day'] = v['일시'].str[3:5]
                v['hour'] = v['일시'].str[6:8]
                v['month'] = v['month'].astype(float)
                v['day'] = v['day'].astype(float)
                v['hour'] = v['hour'].astype(float)
                v = v.drop(['일시'],axis=1)
                
                # x,y
                print('x,y')
                x = v[(a+i03_01) : (a+i03_01+47)]
                x = split_x(x,timesteps)

                print(x)
                y = v[v02].iloc[(a+i03_01+24):(a+i03_01+48)].values
                # y = v[(a+i03_01+24)]
                print(y)
                print(x.shape)
                print(y.shape)
                ...
                model= Sequential()
                model.add(LSTM(32,input_shape=(24,10)))
                # model.add(LSTM(32))
                ...
                es = EarlyStopping(monitor='loss',mode='min',restore_best_weights=True,patience=48)
                model.fit(x,y,epochs=128,verbose=1,callbacks =[es])
                end= time.time()
                print(v,'걸린 시간 : ',round(end -start,2),'초')

                # 4. 평가, 예측
                result = model.evaluate(x,y)
                x_pred = v[(a+i03_01+24):(a+i03_01+48)].values
                print(x_pred.shape)# (24, 10)
                x_pred = x_pred.reshape(1,24,10)
                # print(x_pred)
                y_pred = model.predict(x_pred)
                # r2 = r2_score(y,y_pred)
                # print('r2 : ',r2)
                mae = mean_absolute_error(y[0],y_pred)
                print('mae : ',mae)

                # 제출 
                v[v02].iloc[(a+i03_01+48+1)] = y_pred

                print(f'{i}번째',v.head(100))

                a =  a + 48

        # for i03_02 in range(36) : 
...
x_train = x_train.reshape(x_test.shape[0],timesteps,10)
x_test = x_test.reshape(x_test.shape[0],timesteps,10)
...
# aws_list = []
# for i,v in enumerate(aws_list) : 
#     a= 0
#     for i02 in range(65) :
#         for i03_01 in range(36) : 
#             timesteps = 24
#             # 1. 데이터
#             x = test_aws_data[(a+i) : (a+i+47) ]
#             y = test_aws_data[(a+i+24)+1]
#             x = split_x(x,timesteps)
...
#             from tensorflow.python.keras.callbacks import EarlyStopping, ModelCheckpoint
#             es = EarlyStopping(monitor='loss',mode='min',restore_best_weights=True,patience=8)
#             model.fit(x,y,epochs=48,verbose=1,callbacks =[es])
#             end= time.time()
#             print(v,'걸린 시간 : ',round(end -start,2),'초')

#             # 4. 평가, 예측
#             result = model.evaluate(x,y)
#             x_pred = test_aws[(a+i+24+1):(a+i+48) ]
#             y_pred = model.predict(x_pred)
#             print("model.score : ",result)
#             r2 = r2_score(y,y_pred)
#             print('r2 : ',r2)
